    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_DOWN and plane.state == "flying":
                ...
                # Decrease speed while flying
                plane.rotation = 0
                plane.state = "flying"
                plane.speed -= 1
            elif event.key == pygame.K_RIGHT and plane.state == "flying":
                # Increase speed while flying
                plane.rotation = 0
                plane.state = "flying"
                plane.speed += 1
            elif event.key == pygame.K_UP and plane.state == "descending":
                # Start ascending while descending
                plane.rotation = 0.4
                plane.state = "rising"
            elif event.key == pygame.K_DOWN and plane.state == "rising":
                # Start descending while rising
                plane.rotation = -0.4
                plane.state = "descending"
            elif event.key == pygame.K_RIGHT and (plane.state == "rising" or plane.state == "descending"):
                # Level off the plane
                plane.rotation = 0
                plane.state = "flying"
            elif event.key == pygame.K_UP and plane.state == "flying" and plane.y != CRUISING_ALTITUDE:
                # Can begin ascending if not at cruising altitude
                plane.rotation = 0.4
                plane.state = "rising"
    draw_scene()
